...
def test_item_table_columns():
    ...
    expected_columns = ["id", "name", "quantity",
                        "unit", "created_at", "updated_at"]

    for expected in expected_columns:
        assert expected in column_names, \
            f"Column '{expected}' not found in 'items' table"
    assert len(column_names) == len(expected_columns), \
        f"Unexpected columns exist in the table"
...
def test_item_model_attributes():
    ...
    for column in expected_columns:
        assert hasattr(Item, column), \
            f"Item model missing '{column}' attribute"
